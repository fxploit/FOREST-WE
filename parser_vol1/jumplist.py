        ...
        if options.json_flag == False:
            if options.sqlite_db is None:
                if options.eshost is None:
                    print
                    'No output options used. Use --json OR --txt OR --eshost "ESHOSTIP" --index INDEX'
                    sys.exit(1)
...
class LnkHandler():
    def __init__(self, options, filelist=None, filehandle=None, filename=None,
                 jmp_info={'AppIdCode': None, 'AppIdName': None}):
        self.options = options
...
class JmpFile():
    ...
    def GetAppId(self, jmpfilename, AppIds):
        ...
        pattern = re.compile(
            "([0-9a-f]{16})\.(custom|automatic)Destinations\-ms$")  # 5d696d521de238c3.customDestinations-ms
