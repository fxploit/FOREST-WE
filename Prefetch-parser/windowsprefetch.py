...
from utils import DecompressWin10
...
class Prefetch(object):
    ...
    def prettyPrint(self):
        ...
        print("\n{0}\n{1}\n{0}\n".format(banner, ntpath.basename(self.pFileName))) #파일 이름
        print("Executable Name: {}\n".format(self.executableName))  #실행 파일 이름
        print("Run count: {}\n".format(self.runCount)) #실행 횟수

        if len(self.timestamps) > 1:
            print("Last Executed:")    #마지막 실행
            for timestamp in self.timestamps:
                print("    " + timestamp)
        else:
            print("Last Executed: {}".format(self.timestamps[0]))
        
        print("\nVolume Information:") #볼륨 정보`
        for i in self.volumesInformationArray:
            print("   Volume Name: " + i["Volume Name"].decode("UTF-16", errors="backslashreplace")) #볼륨 이름
            print("   Creation Date: " + i["Creation Date"]) #생성날짜
            print("   Serial Number: " + i["Serial Number"]) # 일련 번호`
            print()

        print("Directory Strings:")  #디렉터리 문자열
        for volume in self.directoryStringsArray:
            for dirstring in enumerate(volume):
                print("{:>4}: {}".format(dirstring[0], dirstring[1]))
        print()
        
        print("Resources Loaded:")  #로드된 리소스
