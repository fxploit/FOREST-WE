...
def create_table(dbname):
    ...
    query_execute(dbname, 'CREATE TABLE Registry_RECmd (id INTEGER PRIMARY KEY AUTOINCREMENT, HivePath varchar, HiveType varchar, Description varchar, Category varchar,\
         KeyPath varchar, ValueName varchar, ValueType varchar, ValueData varchar, ValueData2 varchar, ValueData3 varchar, Comment varchar, Recursive varchar, Deleted varchar,\
              LastWriteTimeStamp varchar, PluginDetailFile varchar)')
...
def insert_Registry_RECmd(dbname, args):
    query_execute(dbname, 'INSERT INTO Registry_RECmd (HivePath, HiveType, Description, Category, KeyPath, ValueName, ValueType, ValueData, ValueData2, ValueData3, Comment,\
         Recursive, Deleted, LastWriteTimeStamp, PluginDetailFile) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', args)
